...
export default function CalendarScreen({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#f4f4f2] flex flex-col md:flex-row overflow-hidden font-sans">
      ...
      <div className="w-full md:w-80 bg-white border-r border-ink/[0.07] flex flex-col overflow-y-auto">
        <div className="p-4 border-b border-ink/[0.07] flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2 bg-ink/[0.05] hover:bg-ink/[0.1] rounded-full text-ink-secondary transition-colors">
              ...
              <div className="space-y-2">
                ...
                {(slotsByDate[selectedDate] || []).length === 0 ? (
                  ...
                  (slotsByDate[selectedDate] || []).map(s => (
                    <div key={s.id} className="flex items-center justify-between bg-white border border-ink/[0.07] p-3 rounded-xl shadow-sm">
                      <div>
                        <div className="font-medium text-ink text-sm flex items-center gap-2">
                          {s.start_time.substring(0, 5)}
                          <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wide ${s.status === 'available' ? 'bg-sage/10 text-sage-dark' : 'bg-orange-100 text-orange-800'}`}>
